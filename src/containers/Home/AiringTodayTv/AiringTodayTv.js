...
import classes from './AiringTodayTv.module.css'
...
class AiringTodayTv extends Component {
  ...
  getMoviesHandler = () => {
    axios.get('https://api.themoviedb.org/3/tv/airing_today?',
      {params: {
        ...
        this.setState({AiringTodayTv: releases});
      }).catch(error => {
  ...
  render() {
    return (
      <div className={classes.AiringToday}>
        ...
        <hr />
        <div className={classes.AiringTodaySeries} ref={this.containerWidthRef}>
          {this.state.AiringTodayTv ? this.state.AiringTodayTv : null}
        </div>
      </div>
    );
