...
import classes from './OnTheAirTv.module.css'
...
class OnTheAirTv extends Component {
  ...
  getMoviesHandler = () => {
    axios.get('https://api.themoviedb.org/3/tv/on_the_air?',
      {params: {
        ...
        this.setState({OnTheAirTv: releases});
      }).catch(error => {
  ...
  render() {
    return (
      <div className={classes.OnTheAir}>
        ...
        <hr />
        <div className={classes.OnTheAirSeries} ref={this.containerWidthRef}>
          {this.state.OnTheAirTv ? this.state.OnTheAirTv : null}
        </div>
        <hr/>
      </div>
    );
