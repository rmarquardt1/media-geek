...
class TopRatedMovies extends Component {
  ...
  getMoviesHandler = () => {
    axios.get('https://api.themoviedb.org/3/movie/top_rated?',
      {params: {
        ...
        this.setState({topRated: releases});
      }).catch(error => {
  ...
  render() {
    return (
      <div className={classes.TopRatedMovies}>
        ...
        <hr />
        <div className={classes.TopRatedMoviesMovies} ref={this.containerWidthRef}>
          {this.state.topRated ? this.state.topRated : null}
        </div>
      </div>
    );
  }
