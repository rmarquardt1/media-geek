...
class TopRatedTv extends Component {
  ...
  getMoviesHandler = () => {
    axios.get('https://api.themoviedb.org/3/tv/top_rated?',
      {params: {
        ...
        this.setState({topRatedTv: releases});
      }).catch(error => {
  ...
  render() {
    return (
      <div className={classes.TopRatedTv}>
        ...
        <hr />
        <div className={classes.TopRatedTvSeries} ref={this.containerWidthRef}>
          {this.state.topRatedTv ? this.state.topRatedTv : null}
        </div>
        <hr/>
      </div>
    );
