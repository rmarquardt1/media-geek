...
import classes from './PickedForYouMovies.module.css';

class PickedForYouMovies extends Component {
  state = {
    pickedMovies: null,
    sliceStart: 0,
    sliceEnd: null,
  ...
  componentWillUpdate(prevProp, prevState) {
    if (prevState.sliceEnd !== this.state.sliceEnd && this.state.sliceEnd) {
      this.getMoviesHandler(localStorage.getItem('userData'));
    }
  }
  ...
  getMoviesHandler = uData => {
    const favs = JSON.parse(uData).favMovieGenres.join('|');
    axios
    ...
    return (
      <div className={classes.PickedMovies}>
        ...
        <div
          className={
            this.state.fadeOut
              ? classes.FadeOut + ' ' + classes.PickedMoviesMovies
              : this.state.fadeIn
              ? classes.FadeIn + ' ' + classes.PickedMoviesMovies
              : classes.PickedMoviesMovies
          }
...
export default PickedForYouMovies;
