  ...
  getMoviesHandler = () => {
    axios.get('https://api.themoviedb.org/3/movie/upcoming?',
      {params: {
        ...
        this.setState({upcomingReleases: releases});
      }).catch(error => {
  ...
  render() {
    return (
      <div className={classes.UpcomingMovies}>
        ...
        <hr />
        <div className={classes.UpcomingMoviesMovies} ref={this.containerWidthRef}>
          {this.state.upcomingReleases ? this.state.upcomingReleases : null}
        </div>
        <hr/>
      </div>
