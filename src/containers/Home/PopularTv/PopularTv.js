...
class PopularTv extends Component {
  ...
  getMoviesHandler = () => {
    axios.get('https://api.themoviedb.org/3/tv/popular?',
      {params: {
        ...
        this.setState({PopularTv: releases});
      }).catch(error => {
  ...
  render() {
    return (
      <div className={classes.PopularTv}>
        ...
        <hr />
        <div className={classes.PopularTvSeries} ref={this.containerWidthRef}>
          {this.state.PopularTv ? this.state.PopularTv : null}
        </div>
        <hr/>
      </div>
