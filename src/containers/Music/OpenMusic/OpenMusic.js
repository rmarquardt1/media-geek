...
class OpenMusic extends Component {
    ...
    if (this.state.selectedAlbumId) {
      this.state.albumResults.data.map(album => {
        if (album.id === this.state.selectedAlbumId) {
          ...
          axios.get('https://cors-anywhere.herokuapp.com/' + album.tracklist)
          .then(response => {
  ...
  resizeHandler = () => {
    ...
    if (this.albumElementRef.current) {
      ...
      const albThumbWidth = windowW < 1367 ? 170 : 220;
      const albElCount = Math.floor(albElWidth / albThumbWidth);
      const albSliceEnd = windowW > 768 ? this.state.albumSliceStart + (albElCount * 2) : windowW < 501 ? this.state.albumSliceStart + 6 : this.state.albumSliceStart + 4;

      const albPageSize = windowW > 768 ? albElCount * 2 : windowW < 501 ? 6 : 4;


      this.setState({
        // albumSliceEnd: this.state.albumSliceStart + albElCount, albumPageSize: albElCount
        albumSliceEnd: albSliceEnd, albumPageSize: albPageSize
      });
    ...
    if (this.relElementRef.current) {
      ...
      const relSliceEnd = windowW > 768 ? this.state.relatedSliceStart + (relElCount * 2) : windowW < 501 ? this.state.relatedSliceStart + 6 : this.state.relatedSliceStart + 4;

      const relPageSize = windowW > 768 ? relElCount * 2 : windowW < 501 ? 6 : 4;


      this.setState({
        // albumSliceEnd: this.state.albumSliceStart + albElCount, albumPageSize: albElCount
        relatedSliceEnd: relSliceEnd, relatedPageSize: relPageSize
      });
    }
  }
  ...
  loadArtistHandler = () => {
    ...
    axios.get('https://cors-anywhere.herokuapp.com/https://api.deezer.com/artist/' + this.props.match.params.id).then(response => {
        this.setState({artistInfo: response.data, showResults: false, loading: false, loadEvents: true, loadRelated: true, loadTopTracks: true});
      }).catch(error => {
        console.log('error ' + error);
      });
  }
  ...
  getTopTracksHandler = () => {
    console.log(this.state.artistInfo.tracklist);
    axios.get('https://cors-anywhere.herokuapp.com/' + this.state.artistInfo.tracklist, {
      params: {
        limit: 5
      }
    }).then(response => {
  ...
  getRelatedHandler = () => {
    axios.get('https://cors-anywhere.herokuapp.com/https://api.deezer.com/artist/' + this.state.artistInfo.id + '/related',{
      params: {
        limit: 10
      }
    })
  ...
  eventClickHandler = (venueId) => {
    axios.get('https://cors-anywhere.herokuapp.com/https://app.ticketmaster.com/discovery/v2/venues/ZFr9jZdavd?', {
      params: {
  ...
  getEventsHandler = (size, page) => {
    axios.get('https://cors-anywhere.herokuapp.com/https://app.ticketmaster.com/discovery/v2/events.json?', {
      params: {
        apikey: 'cVDxtZtz4UAR1m5ISMZ2mZRZSr9W3okH',
    ...
    .then(response => {
      ...
      response.data._embedded.events.map(event => {
        ...
        if (event._embedded) {
          eventInfo.venueName = event._embedded.venues[0].name ? event._embedded.venues[0].name : null;
          eventInfo.venueAddr = event._embedded.venues[0].address ? event._embedded.venues[0].address.line1 : null;
          eventInfo.venueCity = event._embedded.venues[0].city ? event._embedded.venues[0].city.name: null;
          eventInfo.venueCountry = event._embedded.venues[0].country ? event._embedded.venues[0].country.name : null;
          eventInfo.venueCountryCode = event._embedded.venues[0].country ? event._embedded.venues[0].country.countryCode : null;
          eventInfo.venueState = event._embedded.venues[0].state ? event._embedded.venues[0].state.stateCode : null;
          eventInfo.venuePostal = event._embedded.venues[0].postalCode ? event._embedded.venues[0].postalCode : null;
          eventInfo.venueId = event._embedded.venues[0].id ? event._embedded.venues[0].id : null;
        }
        results.push(eventInfo);
      });
      this.setState({eventResults: results, eventTotalPages: response.data.page.totalPages, loadEvents: false});
    }).catch(error => {
      console.log('error: ' + error);
    });
  }
  ...
  albumsNextHandler = () => {
    this.setState({albumsFadeOut: true});
      axios.get('https://cors-anywhere.herokuapp.com/' + this.state.albumResults.next)
      .then(response => {
          this.setState({
            albumResults: response.data, 
            albumsPrev: 'prev' in response.data, 
            albumsNext: 'next' in response.data,
            albumsFadeIn: true
          });
          setTimeout(() => {
            this.setState({albumsFadeIn: true, albumsFadeOut: false});
          }, 300);
      }).catch(error => {
        console.log('error: ' + error);
      });
  ...
  albumsPrevHandler = () => {
    this.setState({albumsFadeOut: true});
    axios.get('https://cors-anywhere.herokuapp.com/' + this.state.albumResults.prev)
    .then(response => {
      this.setState({albumResults: response.data, albumsPrev: 'prev' in response.data, albumsNext: 'next' in response.data});
      setTimeout(() => {
        this.setState({albumsFadeIn: true, albumsFadeOut: false});
      }, 300);
    }).catch(error => {
  ...
  albumCloseHandler = () => {
    this.setState({selectedAlbumId: null, selectedAlbumDetails: null, SelectedAlbumTracks: null});
  }
  ...
  render() {
    const backdrop = this.state.artistInfo ? 'url(' + this.state.artistInfo.picture_xl + ')' : null;
    let albums = null;
    let related = null;

    if (this.state.albumResults) {
      albums = this.state.albumResults.data.slice(this.state.albumSliceStart, this.state.albumSliceEnd).map(album => {
        return (
          <AlbumThumb
            id={album.id}
            key={album.id}
    ...
    if (this.state.relatedArtists) {
      related = this.state.relatedArtists.slice(this.state.relatedSliceStart, this.state.relatedSliceEnd).map(artist => {
        return (
          <NavLink style={{color:'#fff', textDecoration: 'none'}} to={'/Music/Artist/' + artist.id} key={artist.id}>
          <RelatedArtist
            name={artist.name}
            pic={artist.pic}
            key={artist.id}
    ...
    {this.state.artistInfo ?
      <Aux>
        <div className={classes.OpenMusicBackdrop} style={{backgroundImage: backdrop}} />
        ...
        <div className={classes.LeftSide}>
          <div className={classes.OpenMusicPoster}>
            {this.state.artistInfo ? <img src={this.state.artistInfo.picture_big} alt="" /> : null}
          </div>
          {this.state.topFiveResults ? <TopFiveTracks tracks={this.state.topFiveResults} clickPlay={this.musicPlayerHandler} /> : null}
        </div>
        ...
        <div className={classes.OpenMusicPosterMobile}>
          ...
          <div className={uiClasses.SectionHeader}>
            ...
            {!this.state.albumsPrev || this.state.selectedAlbumDetails ? null : 
              <FontAwesomeIcon 
            ...
            {!this.state.albumsNext || this.state.selectedAlbumDetails ? null : 
              <FontAwesomeIcon 
            ...
            <div className={!this.state.albumsFadeOut && !this.state.albumsFadeIn ? classes.AlbumsContainer :
            !this.state.albumsFadeOut && this.state.albumsFadeIn ? classes.AlbumsFadeIn : classes.AlbumsFadeOut} 
          ...
          <div className={uiClasses.SectionHeader}>
            ...
            <h2>Upcoming Events</h2>
              ...
              {this.state.eventPageNumber < (this.state.eventTotalPages - 1) ?
                <FontAwesomeIcon 
                className={uiClasses.NextIcon}
              ...
              <div
              className={!this.state.eventsFadeOut && !this.state.eventsFadeIn ? classes.MusicEventsContainer :
                !this.state.eventsFadeOut && this.state.eventsFadeIn ? classes.MusicEventsFadeIn : classes.MusicEventsFadeOut} 
          ...
          {this.state.playTrack ? <MusicPlayer track={this.state.trackInfo} /> : null}
