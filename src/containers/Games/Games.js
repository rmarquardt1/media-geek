...
// import GameResults from '../../components/Games/Search/GameResults/GameResults';
...
class Games extends Component {
  ...
  searchHandler = (event) => {
    ...
    axios.get('https://www.gamespot.com/api/games/',
    {params: {
      ...
      setTimeout(() => {
        this.setState({queryResults: response.data.results, showResults: true, loading: false, openItem: null});
      }, 300);
      }).catch(error => {
    ...
    if (this.state.queryResults && this.state.showResults) {
      ...
      if (!this.state.error) {
        items = this.state.queryResults
          ...
          // page = <Route path="/results" render={() => <SearchResults items={items} {...this.props}/>} /> ;
        ...
        return (
          ...
          <section className={classes.MediaProject}>
              ...
              {/* {this.state.imgLoading ? <Loader /> : page} */}
