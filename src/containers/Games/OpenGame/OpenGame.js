...
class OpenGame extends Component {
  ...
  componentDidMount() {
    ...
    axios.get(this.props.item.articles_api_url,
    {params: {
    ...
    axios.get(this.props.item.videos_api_url,
    {params: {
      api_key: '0d8aa8ecdf3c37b7298d475400f243d7d9ec6b09',
      format:'json'
  ...
  render() {
    ...
    const genres = this.props.item.genres.map(genre => {
        return(genre.name);
      }).join('\xa0/\xa0');

    return (
      <Aux>
        <div className={classes.OpenMovieBackdrop} style={{backgroundImage: backdrop}} />
        ...
        <div className={classes.OpenMovieInfo}> 
          <div className={classes.OpenMovieDesc}>
            ...
            <p className={classes.OpenMovieGenres}>
              {genres}
            </p>
