...
class Review extends Component {
  state = {
    ...
    summaryAnchor: Math.random().toString().substring(2, 15)
  }
  ...
  openReviewHandler = () => {
    if (!this.state.summaryOpen) {
      this.setState({summaryString: this.state.summaryFull, summaryOpen: !this.state.summaryOpen});
    } else {
      this.setState({summaryString: this.state.summarySubString + '...', summaryOpen: !this.state.summaryOpen});
  ...
  render() {
    return (
        <div className={classes.Review}>
          ...
          <p className={classes.ReviewHeadline}><strong>A Review by {this.props.reviewAuthor}</strong></p>
          </ScrollableAnchor>
          <hr/>
          <p className={classes.ReviewSummary}>{this.state.summaryString}</p>
          {this.state.summaryLength > 900 && !this.state.summaryOpen ?
            <p className={classes.ReadMore} onClick={this.openReviewHandler}>Read More...</p>
            : this.state.summaryLength > 900 && this.state.summaryOpen ?
            <p className={classes.ReadMore} onClick={this.openReviewHandler}>Read Less...</p>
            : null
          }
        </div>
    );
